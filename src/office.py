...
class OfficeDocumentManager:
                    ...
                    if cell.value and isinstance(cell.value, str):
                        for placeholder, replacement in variables.get_all().items():
                            if placeholder in str(cell.value):
                                try:
                                    int(replacement)
                                    cell.value = int(cell.value.replace(placeholder, str(replacement)))
                                except ValueError:
    ...
    def finalize_proposal(self, data: DataManager, scraper: Scraper) -> Path:
        docx2pdf.convert(str(data.files.cover_letter_docx), str(data.files.cover_letter_pdf))
        ...
        if data.system.type == 0:
            scraper.download(data.system.panel.specsheet_url, output_dir=data.files.specsheets_folder)
            scraper.download(data.system.inverter.specsheet_url, output_dir=data.files.specsheets_folder)
        if data.system.type == 1:
            scraper.download(data.system.battery.specsheet_url, output_dir=data.files.specsheets_folder)
    ...
    def generate_email(self, data: DataManager, variables: TemplateManager) -> None:
        outlook = win32.Dispatch("Outlook.Application")
        mail = outlook.CreateItem(0)  # 0: olMailItem
        mail.Subject = f"Mid-State Solar - Proposal for {variables.resolve(r'{{SYSTEM_TYPE}}')}"
    ...
    @staticmethod
    def _get_pv_price(data: DataManager) -> int:
        # Attempt to locate a generated pricing_spreadsheet.xlsx under projects/*/documents
        for _ in data.files.project_folder.iterdir() if data.files.project_folder.exists() else []:
            if data.files.pricing_spreadsheet.exists():
                try:
                    wb = load_workbook(data.files.pricing_spreadsheet, data_only=True, read_only=True)
    ...
    def copy_to_fileserver(self, data: DataManager, settings: SettingsManager) -> Path:
        ...
        destination_folder = Path(settings.files.bids_folder) / f"{data.client.last_name}, {data.client.first_name}"
        destination_folder.mkdir(parents=True, exist_ok=True)

        shutil.copy(data.files.final_proposal, destination_folder / f"{datetime.now().strftime('%y_%m%d')} {data.client.last_name}, {data.client.first_name} {"PV" if data.system.type == 0 else "ESS" if data.system.type == 1 else "PV ESS"} Proposal.pdf")
        shutil.copy(data.files.pricing_spreadsheet, destination_folder / f"{datetime.now().strftime('%y_%m%d')} {data.client.last_name}, {data.client.first_name} {"PV" if data.system.type == 0 else "ESS" if data.system.type == 1 else "PV ESS"} Bid.xlsx")
        shutil.copy(data.files.cover_letter_docx, destination_folder / f"PV ESS Proposal - {data.client.last_name}, {data.client.first_name}.docx")
        shutil.copy(data.files.solargraf_proposal, destination_folder / f"Proposal for - {data.client.name}.pdf")
        shutil.copytree(data.files.images_folder, destination_folder / "images", dirs_exist_ok=True)
