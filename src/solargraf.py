...
class SolargrafScraper:
    ...
    def get_project_data(self, project_id=None, public_id=None) -> dict:
        with sync_playwright() as p:
            ...
            if project_id and public_id:
                page.goto(
                    f"https://app.solargraf.com/projects/{project_id}",
                    wait_until="domcontentloaded",
                )
                self._login_if_possible(page, "Overview")
                preview_public_pattern = re.compile(
                    rf"^https://api\.solargraf\.com/projects/public/{re.escape(str(public_id))}(?:[/?#].*)?$"
                )
                with page.expect_response(
                    lambda response: preview_public_pattern.match(response.url),
                    timeout=120000,
                ) as preview_response_info:
                    page.goto(f"https://app.solargraf.com/preview/{public_id}", wait_until="domcontentloaded")
                ...
                public_id = initial_payload.get("public_id")
                if not public_id:
                    raise RuntimeError("Project response did not include public_id")

                project_id = self._extract_project_id(initial_payload)
                preview_public_pattern = re.compile(
                    rf"^https://api\.solargraf\.com/projects/public/{re.escape(str(public_id))}(?:[/?#].*)?$"
                )
                with page.expect_response(
    ...
    def get_proposal_document(self, project_id: str, public_id: str, proposal_id: str, financial_id: str):
        with sync_playwright() as p:
